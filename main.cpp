...
vector<string> customPuzzle(string &input) 
{
    ...
    {
        rows.push_back(string(1, input.at(i)));
    }
    return rows;
}
...
int main() {
	...
	//create the data structure of nodes
    int queueSize;
    int nodesExpanded = 0;
    while(/* !empty */) //enter loop and ask if nodes is empty
    {
        // pop off the head off nodes
        if (/* goal state*/) //ask if its the goal state, if yes enter if statement and were done 
        {
            cout << "Solution found! " << endl;
            
            cout << "Solution depth was " << /* depth */<< endl;
        ...
        //if not goal state, creates all of current nodes children
